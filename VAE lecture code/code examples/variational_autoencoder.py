...
if 'google.colab' in sys.modules and 'torch' not in sys.modules:
    ...
    accelerator = 'cpu' #cuda_output[0] if exists('/dev/nvidia0') else 'cpu'
...
class MLP(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential([nn.Linear(c*2*7*7 + 10, 128),
                                  nn.ReLU(),
                                  nn.Linear(64, 2)])
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.fc_mu = nn.Linear(in_features=c*2*7*7 + 10, out_features=latent_dims)
        self.fc_logvar = nn.Linear(in_features=c*2*7*7 + 10, out_features=latent_dims)
        # self.mlp = MLP()
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        c = capacity
        self.fc = nn.Linear(in_features=latent_dims + 10, out_features=c*2*7*7)
...
for epoch in range(num_epochs):
    ...
    for image_batch, y in train_dataloader:
        ...
        # vae reconstruction
        image_batch_recon, latent_mu, latent_logvar = vae(image_batch, y)
...
def visualise_output(images, labels, model):
    ...
    with torch.no_grad():
        ...
        images, _, _ = model(images, labels)
        images = images.cpu()
...
visualise_output(images, labels, vae)
...
def interpolation(lambda1, model, img1, y1, img2, y2):
    
    with torch.no_grad():
    
        # latent vector of first image
        img1 = img1.to(device)
        latent_1, _ = model.encoder(img1, y1)

        # latent vector of second image
        img2 = img2.to(device)
        latent_2, _ = model.encoder(img2, y2)
